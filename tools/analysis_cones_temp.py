...
def gHits2cones_byEvtID(file_path, source_MeV):
    ...
    for eventid, grp in grouped:
        apex, direction, E1 = False, False, False
        # Sensor received primary gamma and it interacted TODO: is this correct with radioisotope source?
        if 1 in grp['TrackID'].values or 'RadioactiveDecay' in list(grp['TrackCreatorProcess'].values):
            n_events_primary += 1
            # All primary energy was deposited
            if round(grp['TotalEnergyDeposit'].sum(), 6) == source_MeV:
                # TODO round above is to avoid float precision issues
...
# def gHits2cones_byEvtID(file_path, source, lines_MeV = None):
#
#     if not os.path.isfile(file_path):
#         sys.exit(f"File {file_path} does not exist, probably no hit produced.")
#     else:
...
#         return descendants
#
#     # Case 1: source is monochromatic gamma
#     if source.particle == 'gamma' and source.energy.mono:
...
#             # TODO make order flexible
#
#     # Case 2: source is radio-isotope
#     elif source.particle.startswith('ion'):
#         if not lines_MeV:
#             global_log.error(f"You must enter gamma energies you want to reconstruct when using ion sources. In MeV with 3 digits.")
#             sys.exit()
#         for eventid, grp in grouped:
#             apex, direction, E1 = False, False, False
#             print_hits_short(grp)
#             # Sensor received the (or at least one of the) emitted gamma(s)
#             # 1) in the group, look for rows which have ParticleName == gamma, TrackCreatorProcess == RadioactiveDecay. Store those which have a new TrackId.
#             # 2) for each row/interaction, look for all subsequent interactions
#
#     df = pandas.DataFrame(cones, columns=cones_columns)
#     global_log.debug(f"{n_events_primary} events with primary particle hitting sensor")
#     global_log.debug(f"=> {n_events_full_edep} with full energy deposited in sensor")
